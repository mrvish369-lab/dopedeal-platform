      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {taskTypes.map((task) => (
            <div
              key={task.id}
              className="bg-white rounded-2xl border border-brand-border p-5 flex items-start gap-4 hover:shadow-md hover:shadow-brand-green/10 hover:-translate-y-0.5 transition-all"
            >
              <div
                className={`w-11 h-11 rounded-xl bg-gradient-to-br ${task.color} flex items-center justify-center text-xl shrink-0 shadow-sm`}
              >
                {task.icon}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <div className="font-display font-black text-brand-green-dim text-lg mb-0.5">
                  {task.payout}
                </div>
        ...
        {/* CTA Banner */}
        <div className="bg-gradient-to-br from-brand-forest to-brand-forest-mid rounded-3xl p-7 text-white flex flex-col sm:flex-row items-center justify-between gap-5">
          <div>
          ...
          <Link
            ...
            className="shrink-0 bg-brand-green text-brand-forest font-bold text-sm px-6 py-3 rounded-2xl hover:bg-brand-green-light transition-colors flex items-center gap-2 shadow-lg"
          >
