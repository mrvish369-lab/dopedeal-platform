...
interface ShopQRCardProps {
  ...
  title: string;
  emoji?: string;
  variant?: "quiz" | "product";
}

export const ShopQRCard = ({ qrCode, shopId, shopCode, title, emoji, variant = "quiz" }: ShopQRCardProps) => {
  ...
  const fetchShopQRTraffic = async () => {
    if (!shopId) return;
    setLoading(true);

    try {
      // For quiz QR (variant === "quiz"), we STRICTLY get sessions where batch_id IS NULL
      // For product QR (variant === "product"), we get sessions where batch_id = qrCode.batch_id
      let query = supabase
        .from("sessions")
        .select("id, anonymous_id, device_type, whatsapp_verified, result_type, qr_type, batch_id")
        .eq("shop_id", shopId);

      if (variant === "quiz") {
        // Strictly filter for sessions with NO batch_id (quiz QR scans only)
        query = query.is("batch_id", null);
      } else if (qrCode.batch_id) {
        // For product variant, filter by specific batch
        query = query.eq("batch_id", qrCode.batch_id);
      }

      const { data: sessions, error: sessionError } = await query;

      if (sessionError) {
        console.error("Error fetching shop sessions:", sessionError);
        setLoading(false);
        return;
      }
      ...
      if (sessionIds.length > 0) {
        ...
        if (!eventError && eventData) {
          // Additional filter: ensure events belong to correct session type
          const filteredEvents = eventData.filter((e) => {
            const session = e.session as { batch_id: string | null } | null;
            if (variant === "quiz") {
              return session?.batch_id === null || session?.batch_id === undefined;
            }
            return true;
          });
      ...
      console.error("Error fetching QR traffic:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadQR = () => {
    ...
    const suffix = variant === "quiz" ? "quiz" : "product";
    link.download = `QR_${shopCode || "shop"}_${suffix}.png`;
    link.click();
  };
          ...
          <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-muted/30 transition-colors">
            <div className="flex items-center gap-3">
              <div className={cn(
                ...
                variant === "quiz" ? "bg-amber-500/10" : "bg-primary/10"
              )}>
                {emoji ? (
                  <span className="text-lg">{emoji}</span>
                ) : (
                  <QrCode className={cn("w-5 h-5", variant === "quiz" ? "text-amber-500" : "text-primary")} />
                )}
              </div>
              <div>
              ...
              <>
                ...
                {events.length > 0 ? (
                  ...
                  <div className="text-center py-6 text-muted-foreground">
                    ...
                    <p className="text-sm">No traffic recorded yet for this QR</p>
